        ...
        if form.is_valid():
            ...
            if su:
                ...
                n_su = Subunit.objects.create(parrent_id=lo_id,
                                              ...
                                              con_type=form.cleaned_data['name_type'],
                                              poe=form.cleaned_data['poe'],
                                              ip_addr=form.cleaned_data['ip'],
                                              mac_addr=form.cleaned_data['mac'].replace('-', ':').upper(),
                                              sn=form.cleaned_data['sn'],
                                              inv=form.cleaned_data['inv'],
                                              man_install=form.cleaned_data['man_install'],
                                              date_ent=form.cleaned_data['date_ent'],
                                              date_repl=form.cleaned_data['date_repl'],
                                              stairway=form.cleaned_data['stairway'],
                                              floor=form.cleaned_data['floor'],
                                              prim=form.cleaned_data['prim'],
                                              )
                h_text += 'УД: '+lo.name+'; ' + 'name: '+n_su.name
                to_his([request.user, 13, n_su.id, 1, 0, h_text])
